...
export function summarizePhysiqueForProgram(
  ...
  const confidenceLevel =
    result.guvenPuani >= 80 ? 'high' : result.guvenPuani >= 60 ? 'medium' : 'low';
  return {
    source,
    createdAt,
    confidenceLevel,
    estimateNote: result.kasKutlesiYorumu,
    focusAreas: result.eksikBolgeler.slice(0, 3),
    recommendedExercises: result.odaklanmasiGerekenHareketler.map((item) => item.hareketAdi).slice(0, 4),
    generalSummary: result.generalDurum,
  };
}
...
export function createAIProgramDraftFromSavedPlan(params: {
  ...
  return mergeAIProgramDraft(baseDraft, {
    answers: {
      ...
      priorityMuscles: params.plan.sourceContextSummary.priorityMuscles as AIProgramAnswers['priorityMuscles'],
